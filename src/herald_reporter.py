...
class HeraldMatchReporter:
    ...
    async def _process_and_post_match(self, match_id: int, channels: List) -> bool:
        ...
        try:
            ...
            radiant_embed = create_team_analysis_embed(
                stratz_data.radiant_players, True, player_wl_data
            )
            dire_embed = create_team_analysis_embed(
                stratz_data.dire_players, False, player_wl_data
            )
            ...
            if wl_tasks:
                ...
                for hero_id, wl_result in zip(hero_ids, wl_results):
                    if isinstance(wl_result, tuple):
                        ...
                        logger.warning(
                            f"Failed to fetch W/L for hero_id {hero_id}: {wl_result}"
                        )
