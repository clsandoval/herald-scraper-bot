...
async def send_herald_report():
    ...
    try:
        discord_channel_ids = ["1392724352155254876", "867729394054463548"]

        # Get all channels and validate them
        channels = []
        for channel_id in discord_channel_ids:
            try:
                ...
                if channel:
                    ...
                    logger.info(
                        f"Successfully connected to channel: {channel.name} ({channel_id})"
                    )
                else:
        ...
        if not channels:
            logger.error("No accessible channels found. Cannot send herald report.")
            return

        # Clean up old threads in all channels before starting the herald report
        for channel in channels:
            try:
                await cleanup_old_threads(channel, days_threshold=10)
            except Exception as e:
                logger.error(
                    f"Error cleaning up threads in channel {channel.name}: {str(e)}"
                )
                ...
                if leaver == 0:
                    ...
                    for channel in channels:
                        try:
                            ...
                            match_thread = await match_message.create_thread(
                                name=thread_name
                            )
                            ...
                            logger.info(
                                f"Successfully sent match {match} to channel {channel.name}"
                            )
                        except Exception as e:
                            logger.error(
                                f"Error sending match {match} to channel {channel.name}: {str(e)}"
                            )
